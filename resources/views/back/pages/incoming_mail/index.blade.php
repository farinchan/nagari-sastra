@extends('back.app')
@section('content')
    <div id="kt_content_container" class=" container-xxl ">

        <div class="card card-flush">
            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-4">
                            <span class="path1"></span>
                            <span class="path2"></span>
                        </i>
                        <input type="text" data-kt-ecommerce-category-filter="search"
                            class="form-control form-control-solid w-250px ps-12" placeholder="Cari surat masuk" />
                    </div>
                </div>
                <div class="card-toolbar">
                    <a href="{{ route('back.incoming-mail.create') }}" class="btn btn-primary">
                        <i class="ki-duotone ki-plus fs-2"></i>
                        Tambah Surat Masuk
                    </a>
                </div>
            </div>
            <div class="card-body pt-0">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_category_table">
                    <thead>
                        <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            <th class="min-w-30px">No</th>
                            <th class="min-w-150px">Nomor Surat</th>
                            <th class="min-w-150px">Pengirim</th>
                            <th class="min-w-100px">Tanggal Surat</th>
                            <th class="min-w-200px">Perihal</th>
                            <th class="min-w-80px">Klasifikasi</th>
                            <th class="min-w-50px">File</th>
                            <th class="text-end min-w-70px">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="fw-semibold text-gray-600">
                        @foreach ($list_mail as $index => $mail)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <a href="{{ route('back.incoming-mail.show', $mail->id) }}" class="text-gray-800 text-hover-primary fw-bold">
                                        {{ $mail->nomor_surat }}
                                    </a>
                                </td>
                                <td>{{ $mail->pengirim }}</td>
                                <td>{{ \Carbon\Carbon::parse($mail->tanggal_surat)->format('d M Y') }}</td>
                                <td>{{ Str::limit($mail->perihal, 50) }}</td>
                                <td>
                                    @if($mail->klasifikasi == 'biasa')
                                        <span class="badge badge-light-primary">Biasa</span>
                                    @elseif($mail->klasifikasi == 'penting')
                                        <span class="badge badge-light-warning">Penting</span>
                                    @elseif($mail->klasifikasi == 'rahasia')
                                        <span class="badge badge-light-danger">Rahasia</span>
                                    @elseif($mail->klasifikasi == 'sangat_rahasia')
                                        <span class="badge badge-light-dark">Sangat Rahasia</span>
                                    @endif
                                </td>
                                <td>
                                    @if ($mail->file_surat)
                                        <a href="{{ $mail->getFileSurat() }}" target="_blank">
                                            <i class="ki-duotone ki-file-added text-primary fs-3x" data-bs-toggle="tooltip"
                                                data-bs-placement="right" title="Lihat File">
                                                <span class="path1"></span>
                                                <span class="path2"></span>
                                            </i>
                                        </a>
                                    @else
                                        <i class="ki-duotone ki-file-deleted text-danger fs-3x" data-bs-toggle="tooltip"
                                            data-bs-placement="right" title="Tidak Ada File">
                                            <span class="path1"></span>
                                            <span class="path2"></span>
                                        </i>
                                    @endif
                                </td>
                                <td class="text-end">
                                    <a href="#"
                                        class="btn btn-sm btn-light btn-active-light-primary btn-flex btn-center"
                                        data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Actions
                                        <i class="ki-duotone ki-down fs-5 ms-1"></i></a>
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                        data-kt-menu="true">
                                        <div class="menu-item px-3">
                                            <a href="{{ route('back.incoming-mail.show', $mail->id) }}"
                                                class="menu-link px-3">
                                                Detail</a>
                                        </div>
                                        <div class="menu-item px-3">
                                            <a href="{{ route('back.incoming-mail.edit', $mail->id) }}"
                                                class="menu-link px-3">
                                                Edit</a>
                                        </div>
                                        <div class="menu-item px-3">
                                            <a href="#" class="menu-link px-3" data-bs-toggle="modal"
                                                data-bs-target="#delete_surat_masuk{{ $mail->id }}">
                                                Delete</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    </div>

    @foreach ($list_mail as $mail)
        <div class="modal fade" tabindex="-1" id="delete_surat_masuk{{ $mail->id }}">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title">Hapus Surat Masuk</h3>
                        <div class="btn btn-icon btn-sm btn-active-light-primary ms-2" data-bs-dismiss="modal"
                            aria-label="Close">
                            <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                        </div>
                    </div>
                    <form action="{{ route('back.incoming-mail.destroy', $mail->id) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <div class="modal-body">
                            <div class="mb-3">
                                <p>Apakah Anda yakin ingin menghapus surat masuk
                                    <strong>{{ $mail->nomor_surat }}</strong>?</p>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-danger">Hapus</button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    @endforeach
@endsection

@section('scripts')
    <script src="{{ asset('back/js/custom/apps/ecommerce/catalog/categories.js') }}"></script>
@endsection
